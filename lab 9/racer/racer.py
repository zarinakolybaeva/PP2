...
class Coins(pygame.sprite.Sprite): # создаем дочерний класс Enemy
    def __init__(self): # функция для инициализации
        super().__init__() # инициализируем из родительского класса
        self.radius = random.randint(12, 15) # радиус окружности, который выбирается рандомно из заданного интервала
        self.x = random.randint(15, 280) # начальная координата по оси Оx, выбирается рандомно из заданного интервала
        self.y = 0 # начальная координата по оси Оу
        self.rect = pygame.draw.rect(screen, WHITE, (self.x, self.y, 15, 15)) # описываем квадрат (длина = 6, начальные координаты упомянуты выше)
        self.rect.center = (self.x, self.y) # точка прямоугольника, ее координаты - self.x и self.y (выше описаны)
        self.score = 0 # переменная для счета

    def gen(self): # функция, которая генерирует новые монетки (рандомно выбирая координату по Ох и радиус)
        self.radius = random.randint(12, 15)
        self.x = random.randint(15, 280)
    ...
    def move(self): # функция для передвижения
        ...
        if self.rect.top > 600: # проверяем, если прямоугольник выходит за границу экрана (вниз), то он возвращается наверх, координата по Оу снова равна 0, а координата по Ох и радиус снова выбираются рандомно из заданного интервала
            self.rect.top = 0
            self.x = random.randint(40, 300)
            self.radius = random.randint(12, 15)
            self.y = 0
            ...
            if P1.earn(self.x, self.y): # если эта функция возвращает значение True, то монета (окружность) генерируется заново
                self.rect.top = 0
                self.x = random.randint(40, 300)
                self.radius = random.randint(12, 15)
                self.y = 0
                self.rect.center = (self.x, self.y)
                self.ball = self.radius - 11 # переменная, которая определяет "балл" в зависимости от радиуса
                self.score += self.ball # счетчик увеличивается на балл
