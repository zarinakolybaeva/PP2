# импортируем модули random и pygame
import pygame
import random
...
class Snake: # создаем класс Snake
    ...
    def move(self): # функция по передвижению
        if self.is_add: # если переменная по добавлению новых элементов (окружностей) принимает значение True,то выполняется функция add_to_snake
            self.add_to_snake()
        for i in range(self.size - 1, 0, -1): # пробегаемся от последнего элемента змейки до первого,
            # чтобы каждый предыдущий элемент (окружность) принимал место следующего за ним
            self.elements[i][0] = self.elements[i-1][0]
            self.elements[i][1] = self.elements[i-1][1]
        if self.direction == 'RIGHT': # если направление - "вправо" и змейка не выходит за границы экрана, 
            # то она двигается вправо (координаты элементов по Ох увеличиваются на 5), а в противном случае возвращается влево
            # , на начальную координату по Ох
            if self.elements[0][0] <= 763 and self.elements[0][0] >= 31:
                self.elements[0][0] += self.dx
            else:
                self.elements[0][0] = 31
        elif self.direction == 'LEFT': # если направление - "влево" и змейка не выходит за границы экрана, 
            # то она двигается влево (координаты элементов по Ох уменьшаются на 5), а в противном случае возвращается вправо, на конечную координату по Ох
            if self.elements[0][0] <= 763 and self.elements[0][0] >= 35:
                self.elements[0][0] -= self.dx
            else:
                self.elements[0][0] = 763
        elif self.direction == 'UP': # если направление - "вверх" и змейка не выходит за границы экрана,
            # то она двигается вниз (координаты элементов по Оу уменьшаются на 5), а в противном случае возвращается вниз, на конечную координату по Оу
            if self.elements[0][1] <= 770 and self.elements[0][1] >= 96:
                self.elements[0][1] -= self.dy
            else:
                self.elements[0][1] = 770
        elif self.direction == 'DOWN': # если направление - "вниз" и змейка не выходит за границы экрана,
            # то она двигается вниз (координаты элементов по Оу увеличиваются на 5), а в противном случае возвращается вверх, на начальную координату по Оу
            if self.elements[0][1] <= 770 and self.elements[0][1] >= 96:
                self.elements[0][1] += self.dy
            else:
...
while not done:
    ...
    if S1.eat(F1.x, F1.y): # если еда была съедена (функция eat из класса Snake вернула значение True),
        # то переменная is_add из класса Snake возвращает значение True, и тогда вызывается функция gen из класса Food (генерируем новую еду)
        S1.is_add = True
        F1.gen()
    ...
    pygame.draw.line(screen, BLACK, (20, 10), (780, 10), 1) # линия для отделения верхней части экрана от текста
